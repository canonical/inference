...
func TestConfigureHook(t *testing.T) {
	tests := []struct {
		...
		{name: "zero-prefixed port", host: "127.0.0.1", port: "08400", wantErr: "http.port must be an integer between 1 and 65535"},
...
func TestPostRefreshHookInitializesMissingConfiguration(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			snapDir := t.TempDir()
			commonDir := t.TempDir()
			setLog := filepath.Join(t.TempDir(), "set.log")
...
`
			...
			sets, err := os.ReadFile(setLog)
			if err != nil && !os.IsNotExist(err) {
				t.Fatal(err)
			}
			if string(sets) != test.wantSets {
				t.Fatalf("set configuration %q, want %q", sets, test.wantSets)
			}
